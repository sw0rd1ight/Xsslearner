...
class HtmlParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        for k, v in attrs:
            if v and self.target in v:
                if k == "style" or tag == 'style':
                    self.context.append({"context": "css", "sp":tag,"tag":tag,"start_position": self.getpos()})
                elif k.startswith("on"):
                    self.context.append({"context": "script","sp":k,"tag":tag,"start_position": self.getpos()})
                else:
                    self.context.append({"context": "attr", "sp":k,"tag":tag,"start_position": self.getpos()})
    ...
    def handle_data(self, data):
        ...
        if self.target in data:
            if self.lasttag == 'script':
                self.context.append({"context": "script", "sp":self.lasttag,"tag":self.lasttag,"start_position": self.getpos()})
            else:
                self.context.append({"context": "html","sp":self.lasttag,"tag":self.lasttag,"start_position": self.getpos()})
    ...
    def handle_comment(self, data):
        ...
        if self.target in data:
            self.context.append({"context": "comment", "sp":"","tag":"","start_position": self.getpos()})
