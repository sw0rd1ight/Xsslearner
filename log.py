...
class CustomFormatter(logging.Formatter):
    def format(self, record):
        msg = super().format(record) #先调用夫类的 format方法 得到夫类的结果后 在加上前缀和 end
...
def setup_logger(name='xsslearner'):
    from types import MethodType
    logger = logging.getLogger(name)
    logger.setLevel(logging.DEBUG)
    console_handler = CustomStreamHandler(sys.stdout) #重写了emit方法，用于将\r换成\n
    console_handler.setLevel(log_config[console_log_level]['value'])
    console_handler.setFormatter(CustomFormatter('%(message)s')) ##先调用父类的 format方法 得到父类的结果后 在加上前缀和 end
